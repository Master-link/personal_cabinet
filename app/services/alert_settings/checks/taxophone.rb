# Для услуги Taxophone
# Отправка Email/sms для kind=taxophone, применять только к таким подпискам
  ...
  module Checks
    class Taxophone
      def initialize(subscription)
        ...
        @logger = Logger.new("#{Rails.root}/log/check_taxophone.log")
      end

      def call
        @alert_settings = AlertSetting.find_by(
          client_id: @subscription.client.id,
          alert_type: Service::KIND_TAXOPHONE,
